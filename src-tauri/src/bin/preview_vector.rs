//! Render `rm::vectorize`'s traced contours to a PNG, without a device —
//! the contour-tracing counterpart to `preview_markdown`.
//!
//! Usage: `cargo run --bin preview_vector -- input.png output.png [rm2]`

use rm_bin_lib::rm::device::{PAPER_PRO, RM2};
use rm_bin_lib::rm::vectorize;

fn main() {
    let args: Vec<String> = std::env::args().collect();
    let (Some(src), Some(out)) = (args.get(1), args.get(2)) else {
        eprintln!("usage: preview_vector <input.png> <output.png> [rm2]");
        std::process::exit(2);
    };
    let calib = if args.get(3).map(String::as_str) == Some("rm2") { &RM2 } else { &PAPER_PRO };

    // Raw strokes, not `Plan::preview` — see markdown.rs's `debug_layout`
    // doc for why the preview's own simplification is too coarse to trust
    // for inspecting what actually gets drawn.
    let strokes = vectorize::debug_trace(src, calib).unwrap_or_else(|e| {
        eprintln!("trace failed: {e}");
        std::process::exit(1);
    });
    eprintln!("{} strokes", strokes.len());

    const SCALE: u32 = 2;
    let (w, h) = ((calib.screen_w as u32) * SCALE, (calib.screen_h as u32) * SCALE);
    let mut img = image::GrayImage::from_pixel(w, h, image::Luma([255]));
    let sx = w as f64 / calib.screen_w;
    let sy = h as f64 / calib.screen_h;
    for stroke in &strokes {
        for pair in stroke.windows(2) {
            let (x0, y0) = pair[0];
            let (x1, y1) = pair[1];
            draw_line(&mut img, (x0 * sx) as i64, (y0 * sy) as i64, (x1 * sx) as i64, (y1 * sy) as i64, w, h);
        }
    }
    img.save(out).unwrap_or_else(|e| {
        eprintln!("can't write {out}: {e}");
        std::process::exit(1);
    });
    eprintln!("wrote {out} ({w}x{h})");
}

fn draw_line(img: &mut image::GrayImage, x0: i64, y0: i64, x1: i64, y1: i64, w: u32, h: u32) {
    let (mut x0, mut y0) = (x0, y0);
    let (dx, dy) = ((x1 - x0).abs(), -(y1 - y0).abs());
    let (sx, sy) = (if x0 < x1 { 1 } else { -1 }, if y0 < y1 { 1 } else { -1 });
    let mut err = dx + dy;
    loop {
        if x0 >= 0 && y0 >= 0 && (x0 as u32) < w && (y0 as u32) < h {
            img.put_pixel(x0 as u32, y0 as u32, image::Luma([0]));
        }
        if x0 == x1 && y0 == y1 {
            break;
        }
        let e2 = 2 * err;
        if e2 >= dy {
            err += dy;
            x0 += sx;
        }
        if e2 <= dx {
            err += dx;
            y0 += sy;
        }
    }
}
